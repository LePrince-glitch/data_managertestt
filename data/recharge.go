...
func (recharge Recharge) Add() sql.Result {
	...
	query := fmt.Sprintf(`INSERT INTO recharge VALUES ($1, $2, $3, $4, $5, $6)`)
	r, err := db.Exec(query, recharge.Id, recharge.Volume, recharge.Date_re, recharge.Date_exp, recharge.Auto_re, recharge.Number_id)
	checkIfError(err)
...
func DeleteRecharge(id string) sql.Result {
	...
	query := fmt.Sprintf(`DELETE FROM recharge WHERE "id" = $`)
...
func GetRecharge(id string) Recharge {
	...
	if row.Next() {
		row.Scan(&recharge.Id, &recharge.Volume, &recharge.Date_re, &recharge.Date_exp, &recharge.Auto_re, &recharge.Number_id)
	}
...
func GetRecharges() []Recharge {
	...
	for rows.Next() {
		rows.Scan(&recharge.Id, &recharge.Volume, &recharge.Date_re, &recharge.Date_exp, &recharge.Auto_re, &recharge.Number_id)
